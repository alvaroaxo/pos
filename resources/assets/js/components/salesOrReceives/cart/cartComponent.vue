<template>
  <span>
          ...
          >
            <div class="input-group">
              ...
              <div v-if="customerSearchValue != ''">
                <i
                  class="la la-close position-absolute p-1 customer-search-cancel"
                  @click.prevent="customerSearchValue = ''"
                ></i>
              </div>
              <div v-if="branchSearchValue != ''">
                <i
                  class="la la-close position-absolute p-1 customer-search-cancel"
                  @click.prevent="branchSearchValue = ''"
                ></i>
              </div>
              ...
              >
                <pre-loader
                  v-if="!hideCustomerSearchPreLoader"
                  class="small-loader-container"
                ></pre-loader>
                <div
                  class="px-3 py-1 text-center"
                  v-else-if="
                    hideCustomerSearchPreLoader && customers.length === 0
                  "
                >
                  ...
                  <span v-for="(customer, index) in customers">
                    ...
                    >
                      <h6 class="m-0">
                        ...
                        <small v-show="customer.email != null"
                          ><i class="la la-envelope" />
                          {{ customer.email }}</small
                        >
                        <small v-show="customer.phone_number != null"
                          ><i class="la la-phone-square" />
                          {{ customer.phone_number }}</small
                        >
                      </h6>
                    </a>
              ...
              >
                ...
                <div
                  class="px-3 py-1 text-center"
                  v-else-if="
                    hideCustomerSearchPreLoader && branches.length === 0
                  "
                >
                  {{ trans('lang.no_result_found') }}
                </div>
          ...
          >
            <span>
              <a
                ...
                @shortkey="
                  commonMethodForAccessingShortcut('addCustomerShortcut')
                "
              >
                <i class="la la-user-plus"></i>
              </a>
      ...
      >
        <h6 class="m-0 cart-product-details-parent">
          <div class="cart-product-details-child">
            ...
            <br />
            <small v-show="customer.email != null">
              <i class="la la-envelope"></i> {{ customer.email }}</small
            >
            <small v-show="customer.phone_number != null">
    ...
    >
      ...
      <div
        v-else
        class="cart-item-container py-1"
        v-for="(cartItem, index) in cart"
        :class="{'active-cart-item': cartItem.showItemCollapse}"
      >
        <div class="form-row mx-0 px-1 cart-item">
          <div
            class="col-6 p-0 cart-item-btn"
            @click.prevent="cartItemCollapse(index, cartItem.variantID)"
          >
            <div class="row mx-0 my-auto">
              <div class="col-1 p-0 m-auto">
                <i
                  class="la la-chevron-circle-right la-2x cart-icon"
                  :class="{'cart-icon-rotate': cartItem.showItemCollapse}"
                ></i>
              </div>
              <div class="col-11 my-auto mx-0 pr-0">
                <div
                  class="row mx-0 cart-item-title"
                  :class="{
                    cartProduct:
                      cartItem.productID == activeProductId &&
                      cartItem.variantID == activeVariantId &&
                      cartItem.orderType !== 'discount',
                  }"
                >
                  <div class="col-12 pl-1 p-0 my-auto mx-0">
                    ...
                    <span
                      v-if="
                        ...
                        cartItem.variantTitle != 'default_variant' &&
                        cartItem.orderType === ''
                      "
          ...
          <div class="col-3 d-flex pr-0 pl-0 cart-calculatedPrice">
            ...
            <div
              v-if="salesOrReturnType != 'returns'"
            ...
            >
              <a
                ...
                class="red-text"
                :class="{
                  disabled: cart.length == 1 && !isConnected && offline == 1,
                }"
                @click.prevent="
                  cartItemButtonAction(
                    cartItem.productID,
            ...
            <div v-else class="align-self-center ml-2">
              <a
                ...
                :class="{
                  disabled:
                    (cart.length == 2 && !isConnected && offline == 1) ||
                    (cartItem.orderType == 'discount' &&
                      (cart.length == return_cart_length || cart.length == 2)),
                }"
                @click.prevent="
                  cartItemButtonAction(
                    cartItem.productID,
        ...
        <div
          class="form-row mx-0 px-2 collapse-animation"
          :class="{collapsed: cartItem.showItemCollapse}"
        >
          <div
            class="form-group pl-0 mb-zero"
            :class="[checkDiscount() ? 'col-4' : 'col-6']"
          >
            <label :for="'cart-item-quantity' + index" class="label-in-cart">
              {{ trans('lang.quantity') }}</label
            >
            <payment-input
              :id="'cart-item-quantity' + index"
              :inputValue="numberFormatting(cartItem.quantity)"
              :index="index"
          ...
          <div
            class="form-group pr-0 mb-zero"
            :class="[checkDiscount() ? 'col-4' : 'col-6']"
            v-if="order_type == 'receiving' || manage_price == 1"
          >
            <label :for="'cart-item-price' + index" class="label-in-cart">{{
              trans('lang.price')
            }}</label>
            <payment-input
          ...
          <div class="col-12 p-0 pb-2">
            <label :for="'cart-item-note' + index" class="label-in-cart">{{
              trans('lang.note')
            }}</label>
            <textarea
    ...
    >
      ...
      >
        ...
        <div class="col-3 p-0 text-right">
          <span v-if="overAllDiscount">
            <i class="la la-edit myicon2"></i
            >{{ numberFormat(newOverAllDiscount) }}
          </span>
          <span v-else>
            <i class="la la-edit myicon2"></i
            >{{ numberFormat(newOverAllDiscount) }}
        ...
        <div class="col-6 p-0">
          ...
          <span
            v-if="
              ...
              salesOrReturnType !== 'returns' &&
              order_type == 'sales' &&
              this.sales_or_receiving_type != 'internal-transfer'
            "
          >
            <span class="">( {{ trans('lang.tax') }}</span>
            <span v-if="user.tax_excluded == '0'"
              >{{ trans('lang.included') }})</span
            >
      ...
      <div
        class="row mx-0 px-3 py-2 font-weight-bold border-bottom"
        v-if="
          currentBranch !== null &&
          currentBranch.branch_type === 'restaurant' &&
          salesOrReturnType !== 'returns' &&
          salesOrReceivingType == 'customer'
        "
      >
        ...
        <div class="col-8 p-0 text-right">
          <button
            class="btn btn-restaurant-order-type mr-2"
            :class="{
              'selected-btn-restaurant-order-type':
          ...
          <button
            class="btn btn-restaurant-order-type"
            :class="{
              'selected-btn-restaurant-order-type':
      ...
      <div
        v-if="
          ...
          (currentBranch !== null &&
            currentBranch.branch_type === 'restaurant' &&
            salesOrReceivingType != 'internal' &&
            (order_type === 'receiving' || salesOrReturnType == 'returns'))
        "
        class="p-3 border-bottom"
      >
        <button
          ...
          :disabled="
            ...
            disabledPay() ||
            (!isConnected && offline == 0) ||
            (!isConnected && order_type == 'receiving')
          "
          v-shortkey="payment_short_key"
          @shortkey="commonMethodForAccessingShortcut('payShortcut')"
        >
      ...
      <div v-if="order_type == 'sales'" class="row mx-0">
        <a
          ...
          :class="{
            disabled:
              internalHoldOrders.length == 0 ||
              internalTransferHoldOrders.length == 0 ||
              cart.length != 0 ||
              salesOrReturnType == 'returns',
          }"
        ...
        >
          ...
          <span
            ...
            v-if="countHoldOrder > 0"
            >{{ countHoldOrder }}</span
          >
        </a>
        <a
          v-else-if="salesOrReceivingType == 'customer'"
          href="#"
          class="col-4 p-0 text-center border-right hold-items"
          :class="{
            disabled:
              customerHoldOrders.length == 0 ||
              cart.length != 0 ||
              salesOrReturnType == 'returns',
          }"
          @click.prevent="openHoldOrderModal"
        ...
        <a
          ...
          :class="{
            disabled:
              cart.length == 0 ||
              isEmptyObj(selectedSearchBranch) ||
              (!isConnected && offline == 0),
          }"
        ...
        <a
          ...
          class="col-4 p-0 text-center border-right hold-cart"
          :class="{
            disabled: cart.length == 0 || (!isConnected && offline == 0),
          }"
          @click.prevent="orderHold()"
          v-shortkey="hold_card_item"
          @shortkey="commonMethodForAccessingShortcut('holdCard')"
...
export default {
  ...
  watch: {
    ...
    discount_amount: function (value) {
      if(isset(value) && value != undefined && value != null && value != ''){
        this.discount = value;
      }else{
		this.discount = 0;
	  }
  ...
  methods: {
      ...
      if (instance.customerSearchValue) {
        instance.customers = sortedResults.filter(function (element) {
          let firstName = element.first_name
              .toLowerCase()
              .includes(instance.customerSearchValue.toLowerCase()),
            ...
            if (
              ...
              return (
                ...
                element.email
                  .toLowerCase()
                  .includes(instance.customerSearchValue.toLowerCase()) ||
                element.phone_number.includes(
                  instance.customerSearchValue.toLowerCase()
                )
              );
    ...
    selectCustomer(customer) {
      ...
      if (this.order_type == 'sales') {
        ...
        if (customer.customer_group_discount != undefined) {
          this.allProductDiscount(customer.customer_group_discount, customer.customer_group);
        } else {
          let customerCurrentGroup = customerGroups.find(function (element) {
            return element.id == customer.customer_group;
          });
          this.allProductDiscount(customerCurrentGroup.discount, customerCurrentGroup.customer_group);
        }
      }
      ...
      if (instance.branchSearchValue) {
        instance.branches = sortedResults.filter(function (element) {
          return (
            element.name
              .toLowerCase()
              .includes(instance.branchSearchValue.toLowerCase()) &&
            element.id !== instance.currentBranch.id
          );
        });
      } else {
    ...
    allProductDiscount(value, group, index, unformatted) {
      //console.log(value, group, index, unformatted);
      this.$emit(
        'allProductDiscountFromCart',
        Number(value),
        Number(group),
        index,
        unformatted
    ...
    cartItemButtonAction(cartProductID, cartVariantID, orderType, action) {
      ...
      this.cart.forEach(function (cartItem, index, cartArray) {
        if (
          cartItem.productID == cartProductID &&
          cartItem.variantID == cartVariantID
        ) {
          if (action == '+') {
            cartArray[index].quantity++;

            if (
              cartArray[index].quantity >= cartArray[index].availbleQuantity &&
              instance.order_type == 'sales' &&
              instance.outOfStockProduct == 1
            ) {
            ...
            if (
              ...
              let variantTitle =
                cartArray[index].variantTitle === 'default_variant'
                  ? ''
                  : `(${cartArray[index].variantTitle})`;
              let alertMessage =
                cartArray[index].productTitle +
                ' ' +
                variantTitle +
                ' ' +
                instance.trans('lang.is_out_of_stock');
            ...
            if (
              (instance.order_type === 'sales' &&
                instance.salesOrReturnType === 'returns') ||
              (instance.order_type === 'receiving' &&
                instance.receiveOrReturnType === 'returns')
            ) {
    ...
    setQuantityInCart(value, index) {
      ...
      value = Number(value);
      if (
        this.outOfStockProduct == 1 &&
        value > this.cart[index].availbleQuantity &&
        this.order_type == 'sales'
      ) {
        this.cart[index].quantity = 0;
        this.isDisableCartPlusBtn = true;
        let variantTitle =
          this.cart[index].variantTitle === 'default_variant'
            ? ''
            : `(${this.cart[index].variantTitle})`;
        let alertMessage =
          this.cart[index].productTitle +
          ' ' +
          variantTitle +
          '' +
          this.trans('lang.is_out_of_stock') +
          this.trans('lang.available_quantity') +
          this.cart[index].availbleQuantity;
      ...
      if (
        this.cart[index].quantity > this.cart[index].availbleQuantity &&
        this.order_type == 'sales'
      ) {
        let variantTitle =
          this.cart[index].variantTitle === 'default_variant'
    ...
    down() {
      if (this.open) {
        if (
          this.highlightIndex < this.customers.length - 1 ||
          this.highlightIndex < this.branches.length - 1
        ) {
          this.highlightIndex++;
        }
      } else {
    ...
    commonMethodForAccessingShortcut(data) {
      if (
        data == 'addCustomerShortcut' &&
        this.shortcutKeyInfo.addCustomerShortcut.status == 1 &&
        this.shortcutStatus == 1
      ) {
        $('#customer-add-edit-modal').modal('show');
      ...
      if (
        data == 'payShortcut' &&
        this.shortcutKeyInfo.payShortcut.status == 1 &&
        this.shortcutStatus == 1 &&
        this.cart.length != 0
      ) {
      ...
      if (
        data == 'holdCard' &&
        this.shortcutKeyInfo.holdCardShortcut.status == 1 &&
        this.shortcutStatus == 1 &&
        this.cart.length != 0
      ) {
        if (this.salesOrReceivingType == 'internal') {
      ...
      if (
        data == 'cancelCarditem' &&
        this.shortcutKeyInfo.cancelCardShortcut.status == 1 &&
        this.shortcutStatus == 1 &&
        this.cart.length != 0
      ) {
