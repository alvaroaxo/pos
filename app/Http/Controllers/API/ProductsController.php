...
class ProductsController extends Controller
{
    ...
    private function variantInsertion($request, $productId, $branch)
    {
        ...
        $sallingPricePromotor = $request->sallingPricePromotor ? $request->sallingPricePromotor : 0;
        $sallingPriceMayorista = $request->sallingPriceMayorista ? $request->sallingPriceMayorista : 0;
        ...
        if (!empty($variantDetails)) {
            ...
            foreach ($variantDetails as $item) {
                ...
                array_push($variantInsertValue, [
                    ...
                    'selling_price_promotor' => $item['sellingPricePromotor'] ? $item['sellingPricePromotor'] : 0,
                    'selling_price_mayorist' => $item['sellingPriceMayorista'] ? $item['sellingPriceMayorista'] : 0,
            ...
            $storableData = ['product_id' => $productId->id,
                ...
                'selling_price_promotor' => $sallingPricePromotor,
                'selling_price_mayorist' => $sallingPriceMayorista,
                'sku' => $sku,
                're_order' => $reorder,
                'bar_code' => $barcode,
                'enabled' => $enabled
    ...
    public function editProduct(Request $request, $id)
    {
        ...
        if ($request->type == 0) {
            ...
            $variantData['selling_price_promotor'] = $request->sallingPricePromotor ? $request->sallingPricePromotor : 0;
            $variantData['selling_price_mayorist'] = $request->sallingPriceMayorista ? $request->sallingPriceMayorista : 0;
        ...
        foreach ($variantDetails as $index => $item) {
            ...
            if (!$isDuplicate) {
                ...
                if (!$item['enabled']) {
                    ...
                    if ($item['sellingPricePromotor'] == null) {
                        $variantInsertValue['selling_price_promotor'] = 0;
                    }
                    if ($item['sellingPriceMayorista'] == null) {
                        $variantInsertValue['selling_price_mayorist'] = 0;
                    }
        ...
        if ($isValid == true) {
            foreach ($request->importData as $index => $product) {
                ...
                $productData = [
                    ...
                    'selling_price_promotor' => $product['SELLING-PRICE-PROMOTOR'],
...
//        $count = ProductVariant::alreadyExisted($productData['sku'], $productData['bar_code'], $productData['title'], $productData['product_type']);
        ...
        if ($count == 0 && !empty($productData['title'])) {
            ...
            $variantData = array(
                ...
                'selling_price_promotor' => $productData['selling_price_promotor'],
                'enabled' => 1,
    ...
    private function insertProductAttributeForImport($data)
    {
        ...
        $data['taxable'] = 0;
        $data['tax_type'] = 'custom';
        unset($data['variant_title'], $data['purchase_price'], $data['selling_price'], $data['selling_price_promotor'],$data['attribute_value'], $data['variant_details'], $data['sku'], $data['bar_code'], $data['re_order'], $data['name'], $data['short_name']);
